...
2. **Semantic Embedding & Scoring** (`src/embed.py`): Utilizes the ultra-fast CPU-optimized `minishlab/potion-base-32M` (Model2Vec) to generate dense vector embeddings of the profiles and the JD, executing cosine similarity scoring in seconds.
3. **Filtering, Ranking & Rationale** (`src/rank.py`): Ranks the candidates by their vector similarity score. The top 100 candidates are then processed by a quantized local Small Language Model (SLM) `LiquidAI/LFM2.5-1.2B-Instruct` using `llama.cpp` to dynamically generate a 1-sentence factual justification of their rank based on their profile data.
...
- `src/checker.py`: Script to structurally validate the output `submission.csv` against competition constraints.
